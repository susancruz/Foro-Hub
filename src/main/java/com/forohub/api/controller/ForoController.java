...
public class ForoController {
	...
	@GetMapping("/{id}")
	@Operation(summary = "Foro con mensajes")
    public ResponseEntity<DatosObtenerForo> obtenerForo(@PathVariable Long id, @PageableDefault(size = 2) Pageable paginacion) {
        ...
        if(id == null || !foroRepository.existsById(id)) {
        	 throw new ValidationException("Sin resultado");
        }
	...
	@GetMapping
	@Operation(summary = "Lista de foros")
	...
	@PutMapping
    @Transactional
    @Operation(summary = "Actualiza foro existente")
    ...
    @DeleteMapping("/{id}")
    @Transactional
    @Operation(summary = "Elimina un foro")
